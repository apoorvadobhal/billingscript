...
public class BillDAO {
	...
	public List<BillModel> Invoicebreakup(int custid,int invoiceid)
	{
		...
		try
		{
			PreparedStatement ps1=con.prepareStatement("select a.prod_id,b.ps_desc,a.amount,b.ps_quantity,a.charge_start_date,a.charge_end_date,a.tax from invoice_details a,product_services b where a.prod_id=b.prod_id and a.service_id=b.service_id and a.order_id=b.order_id and a.cust_id=? and a.invoice_id=? and finalise=? ");
			String finalise="y";
			ps1.setInt(1,custid);
	...
	public List<BillModel> Cust_Info(int custid)
	{
		...
		try
		{
			PreparedStatement ps2=con.prepareStatement("select cust_name,cust_mobile_no from customer where cust_id=? ");
			ps2.setInt(1,custid);
